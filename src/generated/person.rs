...
pub const SELECT_ALL_QUERY: &str = "select name, age, email, json_example from person";
...
pub const INSERT_QUERY: &str =
    "insert into person(name, age, email, json_example) values (?, ?, ?, ?)";
#[doc = r" The query to insert a unique row in the table with a TTL"]
pub const INSERT_TTL_QUERY: &str =
    "insert into person(name, age, email, json_example) values (?, ?, ?, ?) using ttl ?";
...
pub const SELECT_UNIQUE_QUERY: &str =
    "select name, age, email, json_example from person where name = ? and age = ?";
#[doc = "The query to update column email"]
pub const UPDATE_EMAIL_QUERY: &str = "update person set email = ? where name = ? and age = ?";
#[doc = "The query to update column json_example"]
pub const UPDATE_JSON_EXAMPLE_QUERY: &str =
    "update person set json_example = ? where name = ? and age = ?";
...
impl From<PersonRef<'_>> for Person {
    ...
    fn from(f: PersonRef<'_>) -> Person {
        Person {
            ...
            json_example: f.json_example.to_string(),
        }
    }
}
impl Person {
...
impl<'a> PersonRef<'a> {
    ...
    pub fn insert_qv(&self) -> Result<Insert, SerializeValuesError> {
        ...
        serialized.add_value(&self.json_example)?;
        Ok(Insert::new(Qv {
            query: INSERT_QUERY,
            values: serialized,
        }))
...
impl Person {
    ...
    pub fn in_memory_update(&mut self, update: UpdatableColumn) {
        match update {
            ...
            UpdatableColumn::JsonExample(val) => {
                self.json_example = val;
            }
        }
    }
...
impl PrimaryKeyRef<'_> {
    #[doc = "Returns a struct that can perform an update operation for column json_example"]
    pub fn update_json_example_qv(&self, val: &str) -> Result<Update, SerializeValuesError> {
        let mut serialized_values = SerializedValues::with_capacity(3usize);
        serialized_values.add_value(&val)?;
        serialized_values.add_value(&self.name)?;
        serialized_values.add_value(&self.age)?;
        Ok(Update::new(Qv {
            query: UPDATE_JSON_EXAMPLE_QUERY,
            values: serialized_values,
        }))
    }
    #[doc = "Performs an update operation for column json_example"]
    pub async fn update_json_example(
        &self,
        session: &CachingSession,
        val: &str,
    ) -> ScyllaQueryResult {
        ...
        self.update_json_example_qv(val)?.update(session).await
    }
}
impl PrimaryKeyRef<'_> {
    ...
    pub fn update_dyn_qv(
        ...
        match val {
            ...
            UpdatableColumnRef::JsonExample(val) => self.update_json_example_qv(val),
        }
    }
    ...
    pub fn update_dyn_multiple_qv(
        ...
        for v in val {
            match v {
                ...
                UpdatableColumnRef::JsonExample(v) => {
                    query.push(concat!(stringify!(json_example), " = ?"));
                    serialized_values.add_value(v)?;
                }
            }
        }
...
impl UpdatableColumn {
    ...
    pub fn to_ref(&self) -> UpdatableColumnRef<'_> {
        match &self {
            ...
            UpdatableColumn::JsonExample(v) => UpdatableColumnRef::JsonExample(v),
        }
    }
}
...
impl From<UpdatableColumnRef<'_>> for UpdatableColumn {
    ...
    fn from(f: UpdatableColumnRef<'_>) -> UpdatableColumn {
        match f {
            ...
            UpdatableColumnRef::JsonExample(v) => UpdatableColumn::JsonExample(v.to_string()),
        }
    }
}
...
impl Person {
    ...
    #[doc = "Creates the updatable column json_example which can be used to update it in the database"]
    pub fn updatable_column_json_example(&self) -> UpdatableColumnRef {
        UpdatableColumnRef::JsonExample(&self.json_example)
    }
}
